...
class ParcelInvoiceController extends Controller
{
    ...
    public function createOrEdit($id=null)
    {
        if($id){
            ...
            $data['parcelInvoiceDetails'] = ParcelInvoiceDetails::leftJoin('items','items.id','sale_details.item_id')
    ...
    public function inovice($id, $print=null)
    {
        ...
        $data['master'] = ParcelInvoice::join('customers','customers.id','=','sales.customer_id')
                            ...
                            ->first()
                            ->toArray();
        $data['details'] = ParcelInvoiceDetails::leftJoin('items','items.id','sale_details.item_id')
                            ->leftJoin('units','units.id','items.unit_id')
                            ->leftJoin('bike_services','bike_services.id','sale_details.service_id')
    ...
    public function payment(Request $request)
    {
        ...
        $sale = ParcelInvoice::find($request->sale_id);
    ...
    public function update(Request $request,$id)
    {
        ...
        try {
            ...
            ParcelInvoiceDetails::where('sale_id', $id)->delete();
    ...
    public function approve($id)
    {
        try {
            ...
            $sale = ParcelInvoice::findOrFail($id);
            ...
            $saleDetails = ParcelInvoiceDetails::where('sale_id', $id)->get();
            ...
            foreach ($saleDetails as $key => $sd) {
                if ($sd->item_type == 0) {
                    ...
                    ParcelInvoiceDetails::find($sd->id)->update(
                        [
