    ...
    public function supplierLedger(Request $request)
    {
        ...
        if ($request->isMethod("POST")) {
            ...
            if ($print=='true') {
                ...
                return view('backend.reports.supplier-ledgers.print', compact('data'));
            }else {
                $data['suppliers'] = Supplier::where('status',1)->orderBy('name','asc')->get()->toArray();

                return view('backend.reports.supplier-ledgers.index', compact('data'));
            }
        }
    }
    ...
    public function purchaseReport(Request $request)
    {
        ...
        if ($request->isMethod("POST")) {
            ...
            if ($print=='true') {
                ...
                return view('backend.reports.purchase-reports.print', compact('data'));
            }else {
                return view('backend.reports.purchase-reports.index', compact('data'));
            }
        }
    }
    ...
    public function salesReport(Request $request)
    {
        ...
        if ($request->isMethod("POST")) {
            ...
            if ($print=='true') {
                ...
                $data['lists'] = $query->get()->toArray();
                $datat['purchaseSummery'] = $query2->select(DB::raw('SUM(total_price) as total_price, SUM(vat_tax) as total_vat, SUM(discount) as total_discount, SUM(total_payable) as total_payable, SUM(paid_amount) as totatl_paid, SUM(total_payable - paid_amount) as total_due'))->first();
                return view('backend.reports.sales-reports.print', compact('data'));
            }else {
                return view('backend.reports.sales-reports.index', compact('data'));
            }
        }
    }
